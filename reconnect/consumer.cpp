...
int MsgConsumer::init(char *topic, char *brokers, char *partitions, char *username, char *password, char *groupId, consumer_callback consumer_cb, void * param_cb)
{
    std::cout << "=======================" << std::endl;
    int ret = 0;
    ...
    if (username != NULL) {
        ...
        rd_kafka_conf_set(m_kafka_conf, "sasl.username", username, errstr,
                          sizeof(errstr));
        rd_kafka_conf_set(m_kafka_conf, "sasl.password", password, errstr,
                          sizeof(errstr));
...
void MsgConsumer::err_cb(rd_kafka_t *rk, int err, const char *reason, void *opaque)
{
    printf("%% ERROR CALLBACK: %s: %s: %s\n", rd_kafka_name(rk), rd_kafka_err2str((rd_kafka_resp_err_t)err), reason);
}


void MsgConsumer::throttle_cb(rd_kafka_t *rk, const char *broker_name, int32_t broker_id, int throttle_time_ms, void *opaque)
{
    printf("%% THROTTLED %dms by %s (%d)\n", throttle_time_ms, broker_name, broker_id);
}


void MsgConsumer::offset_commit_cb(rd_kafka_t *rk, rd_kafka_resp_err_t err, rd_kafka_topic_partition_list_t *offsets, void *opaque)
{
    int i;
    int verbosity = 1;
...
int MsgConsumer::stats_cb(rd_kafka_t *rk, char *json, size_t json_len, void *opaque)
{
    printf("%s\n", json);
    return 0;
}
...
void MsgConsumer::logger(const rd_kafka_t *rk, int level, const char *fac, const char *buf)
{
    fprintf(stdout, "RDKAFKA-%i-%s: %s: %s\n", level, fac, rd_kafka_name(rk), buf);
}


void MsgConsumer::msg_consume(rd_kafka_message_t *rkmessage, void *opaque)
{
    MsgConsumer * p = (MsgConsumer *)opaque;
