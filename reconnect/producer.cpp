...
MsgProducer::MsgProducer() : m_partition(0), m_kafka_handle(), m_kafka_conf(), m_kafka_topic(), m_kafka_topic_conf(), m_kafka_topic_partition_list()
{
}
...
int MsgProducer::init(char *topic, char *brokers, int partition) {
    int ret = 0;
...
int MsgProducer::SendToMqDirect(const std::string &strKey, const char *buffer, size_t buf_len) {
    ...
    ret = rd_kafka_produce(m_kafka_topic, m_partition, RD_KAFKA_MSG_F_COPY | RD_KAFKA_MSG_F_FREE, (void *) buffer, buf_len, NULL, 0, NULL);
...
void MsgProducer::throttle_cb(rd_kafka_t *rk, const char *broker_name, int32_t broker_id, int throttle_time_ms, void *opaque)
{
    printf(">>> %% THROTTLED %dms by %s (%d)\n", throttle_time_ms, broker_name, broker_id);
}

void MsgProducer::offset_commit_cb(rd_kafka_t *rk, rd_kafka_resp_err_t err, rd_kafka_topic_partition_list_t *offsets, void *opaque)
{
    int i;
    int verbosity = 1;
